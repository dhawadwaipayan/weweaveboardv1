...
interface SketchSubBarProps {
  onCancel: () => void;
  onGenerate: (details: string) => void;
}
export const SketchSubBar: React.FC<SketchSubBarProps> = ({
  onCancel,
  onGenerate
}) => {
  ...
  return <div className="w-[472px] h-[45px] bg-[#1a1a1a] border border-[#373737] rounded-xl flex items-center justify-between mb-2.5 mx-0 py-[24px] px-[8px]">
      {/* Additional details input */}
      <div className="flex-1 mr-6 px-[8px]">
      ...
      {/* Cancel button */}
      <button onClick={handleCancel} className="flex items-center gap-2 px-4 py-2 text-white hover:text-[#E1FF00] transition-colors">
      ...
      {/* Generate button */}
      <button onClick={handleGenerate} className="flex items-center gap-2 px-4 py-2 text-[#E1FF00] hover:text-white transition-colors">
        <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-[30px] h-[30px]">
